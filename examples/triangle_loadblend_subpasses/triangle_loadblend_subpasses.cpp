...
public:
	...
	// Same pipeline for the render pass' second subpass
	VkPipeline pipelineSubpass1{ VK_NULL_HANDLE };
	...
	{
		title = "Triangle load-blend, a 2-subpass render pass recorded twice";
	...
	void setupRenderPass() override
	{
		...
		// Subpass 1 blends onto what subpass 0 wrote
		...
		// Two subpasses with the same attachments, one draw each
		const std::array<VkSubpassDescription, 2> subpassDescriptions{ subpassDescription, subpassDescription };
	...
	void createPipelines()
	{
		...
		VK_CHECK_RESULT(vkCreateGraphicsPipelines(device, pipelineCache, 1, &pipelineCI, nullptr, &pipelineSubpass1));
	...
	void render() override
	{
		...
		// A dump window that starts inside the first instance and ends in the second runs past the end of a
		...
		for (uint32_t instance = 0; instance < 2; instance++) {
			...
			vkCmdBindPipeline(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, pipelineSubpass1);
			viewport.x = (float)width * 0.15f;
			vkCmdSetViewport(commandBuffer, 0, 1, &viewport);
			vkCmdDrawIndexed(commandBuffer, indices.count, 1, 0, 0, 0);
