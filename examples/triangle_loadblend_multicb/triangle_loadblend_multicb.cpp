...
// One command buffer clears the swapchain image, the next two each load it and blend a triangle on top
constexpr uint32_t kCommandBuffersPerFrame = 3;
...
public:
	...
	std::array<VkCommandBuffer, MAX_CONCURRENT_FRAMES * kCommandBuffersPerFrame> commandBuffers{};
	// Render pass that clears the colour attachment, used by the first command buffer of each frame
	...
	{
		title = "Triangle load-blend across three command buffers";
	...
	void createCommandBuffers()
	{
		...
		VkCommandBufferAllocateInfo cmdBufAllocateInfo = vks::initializers::commandBufferAllocateInfo(commandPool, VK_COMMAND_BUFFER_LEVEL_PRIMARY, MAX_CONCURRENT_FRAMES * kCommandBuffersPerFrame);
	...
	void setupRenderPass() override
	{
		...
		// colour attachment instead of loading it. The first command buffer of each frame uses it to put
		// the swapchain image into a known state.
	...
	void render() override
	{
		...
		// Three command buffers per frame, the shape reported in issue 3258: the first clears the
		// swapchain image, the next two each load it and additively blend the same triangle on top.
		VkCommandBuffer* const frameCommandBuffers = &commandBuffers[currentFrame * kCommandBuffersPerFrame];
		for (uint32_t i = 0; i < kCommandBuffersPerFrame; i++) {
			vkResetCommandBuffer(frameCommandBuffers[i], 0);
		}

		VkCommandBufferBeginInfo cmdBufInfo{};
		cmdBufInfo.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO;

		// Clear to black, so the blended triangle is the only thing in the attachment and one, two and the
		// buggy three applications of it are exactly one, two and three times the first draw's dump
		...
		// Command buffer 0: clear the swapchain image. It draws nothing, so it is never a dump target.
		VK_CHECK_RESULT(vkBeginCommandBuffer(frameCommandBuffers[0], &cmdBufInfo));
		vkCmdBeginRenderPass(frameCommandBuffers[0], &renderPassBeginInfo, VK_SUBPASS_CONTENTS_INLINE);
		vkCmdEndRenderPass(frameCommandBuffers[0]);
		VK_CHECK_RESULT(vkEndCommandBuffer(frameCommandBuffers[0]));

		// Command buffers 1 and 2: identical, each loads the attachment and blends the triangle at the same
		// place. The dump of the draw in command buffer 2 must hold the triangle exactly twice, so it is
		// exactly twice as bright as the dump of the draw in command buffer 1.
		renderPassBeginInfo.renderPass = renderPass;
		for (uint32_t i = 1; i < kCommandBuffersPerFrame; i++) {
			const VkCommandBuffer commandBuffer = frameCommandBuffers[i];
		...
		submitInfo.pCommandBuffers = frameCommandBuffers;   // Command buffers(s) to execute in this batch (submission)
		submitInfo.commandBufferCount = kCommandBuffersPerFrame;
