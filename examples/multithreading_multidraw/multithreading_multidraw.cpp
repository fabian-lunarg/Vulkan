...
public:
	...
	{
		title = "Multi threaded command buffer, four draws per secondary";
	...
	void threadRenderCode(uint32_t threadIndex, uint32_t cmdBufferIndex, VkCommandBufferInheritanceInfo inheritanceInfo)
	{
		...
		// Several draws per secondary, so that a dump can target one that is not the last
		for (uint32_t d = 0; d < 4; ++d) {
			ThreadPushConstantBlock block = thread->pushConstBlock[cmdBufferIndex];
			block.mvp = matrices.projection * matrices.view *
				glm::translate(glm::mat4(1.0f), glm::vec3(0.0f, 4.0f * float(d), 0.0f)) * objectData->model;
			vkCmdPushConstants(cmdBuffer, pipelineLayout, VK_SHADER_STAGE_VERTEX_BIT, 0,
				sizeof(ThreadPushConstantBlock), &block);
